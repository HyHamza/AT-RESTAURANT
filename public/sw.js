// AT Restaurant - Service Worker v8 - Production Ready
...
self.addEventListener('install', (event) => {
  console.log('[SW] Installing v8 - Production Ready...')
...
self.addEventListener('activate', (event) => {
  console.log('[SW] Activating v8 - Production Ready...')
  ...
  event.waitUntil(
    (async () => {
      ...
      console.log('[SW] v8 fully activated and operational')
    })()
  )
})
...
self.addEventListener('fetch', (event) => {
  ...
  // Video - CRITICAL: Network-first, NEVER block on cache operations
  // Large videos (16MB) must NEVER block the response or hang the app
  if (request.destination === 'video' || url.pathname.match(/\.(mp4|webm)$/)) {
    event.respondWith(
      (async () => {
        try {
          // CRITICAL: Fetch from network FIRST (don't check cache first to avoid hangs)
          ...
          if (response.status === 200 && url.origin === self.location.origin) {
            ...
            caches.open(VIDEO_CACHE).then(cache => {
              ...
              // Notify clients that video is now cached
              ...
              console.warn('[SW] Video cache failed (quota exceeded?):', err.message)
            })
          } else if (response.status === 206) {
            console.log('[SW] Partial response (206), not caching:', url.pathname)
          }
          
          // Return response immediately (don't wait for cache)
          return response
        } catch (error) {
          // Network failed - try cache as fallback (only when offline)
          console.log('[SW] Video network failed, trying cache:', error.message)
          try {
...
console.log('[SW] v8 Production Ready - Loaded successfully')
